...
- 🎯 Entry points to execute your program using `python -m <dummy_fastapi>` or `$ dummy_fastapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/mcmasty/dummy_fastapi/branch/main/graph/badge.svg?token=dummy_fastapi_token_here)](https://codecov.io/gh/mcmasty/dummy_fastapi)
[![CI](https://github.com/mcmasty/dummy_fastapi/actions/workflows/main.yml/badge.svg)](https://github.com/mcmasty/dummy_fastapi/actions/workflows/main.yml)

Awesome dummy_fastapi created by mcmasty

## Install

from source
```bash
git clone https://github.com/mcmasty/dummy_fastapi dummy_fastapi
cd dummy_fastapi
make install
```
...
```bash
pip install dummy_fastapi
```

## Executing

```bash
$ dummy_fastapi run --port 8080
```

or

```bash
python -m dummy_fastapi run --port 8080
```

or

```bash
$ uvicorn dummy_fastapi:app
```
...
```bash
❯ dummy_fastapi --help
Usage: dummy_fastapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ dummy_fastapi create-user --help
Usage: dummy_fastapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
dummy_fastapi create-user admin admin --superuser
```
...
Run with `dummy_fastapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `dummy_fastapi create-user`.
...
-----------------------------------------------------
dummy_fastapi/__init__.py              4      0   100%
dummy_fastapi/app.py                  16      1    94%
dummy_fastapi/cli.py                  21      0   100%
dummy_fastapi/config.py                5      0   100%
dummy_fastapi/db.py                   10      0   100%
dummy_fastapi/models/__init__.py       0      0   100%
dummy_fastapi/models/content.py       47      1    98%
dummy_fastapi/routes/__init__.py      11      0   100%
dummy_fastapi/routes/content.py       52     25    52%
dummy_fastapi/routes/security.py      15      1    93%
dummy_fastapi/routes/user.py          52     26    50%
dummy_fastapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from dummy_fastapi.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in dummy_fastapi/default.toml.

### As environment variables
```bash
export dummy_fastapi_KEY=value
export dummy_fastapi_KEY="@int 42"
export dummy_fastapi_KEY="@jinja {{ this.db.uri }}"
export dummy_fastapi_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
dummy_fastapi_ENV=production dummy_fastapi run
```
